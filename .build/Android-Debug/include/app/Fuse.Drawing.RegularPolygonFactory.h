// This file was generated based on 'C:\ProgramData\Uno\Packages\Fuse.Drawing.Paths\0.1.0\$.uno'.
// WARNING: Changes might be lost if you edit this file directly.

#ifndef __APP_FUSE_DRAWING_REGULAR_POLYGON_FACTORY_H__
#define __APP_FUSE_DRAWING_REGULAR_POLYGON_FACTORY_H__

#include <Uno/Uno.h>
namespace app { namespace Fuse { namespace Drawing { struct PathGeometry; } } }

namespace app {
namespace Fuse {
namespace Drawing {

struct RegularPolygonFactory__uType : ::uClassType
{
};

RegularPolygonFactory__uType* RegularPolygonFactory__typeof();

::app::Fuse::Drawing::PathGeometry* RegularPolygonFactory__AppendTo(::uStatic* __this, ::app::Fuse::Drawing::PathGeometry* self, int Sides, float Radius);

}}}


#endif
