...
PATH="C:/ProgramData/Uno/SDKs/AndroidSDK/platform-tools:$PATH"
...
adb devices | tail -n +2 | cut -sf 1 | xargs -I {} adb -s {} uninstall com.MyMineral

echo ""
echo "# Installing new version of APK"
adb devices | tail -n +2 | cut -sf 1 | xargs -I {} adb -s {} install -r bin/MyMineral-debug.apk

echo ""
echo "# Starting APK on device"
adb devices | tail -n +2 | cut -sf 1 | xargs -I {} adb -s {} shell am start -a android.intent.action.MAIN -c [android.intent.category.LAUNCHER] -f 0x10200000 -n com.MyMineral/com.MyMineral.MyMineral
